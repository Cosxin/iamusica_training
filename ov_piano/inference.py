...
class OnsetVelocityFrameDecoder(torch.nn.Module):
    """Decode onset, velocity and frame probabilities into complete notes.

    Onsets are the only mechanism that starts a note. Frame activity controls
    its release, with debounce to prevent a single low-confidence frame from
    truncating a sustained note. A repeated same-pitch onset closes the
    previous note at the reattack.
    """

    def __init__(self, num_keys, frame_off_threshold=0.5,
                 release_debounce_frames=3, min_note_frames=2,
                 max_note_frames=None, **onset_decoder_kwargs):
        super().__init__()
        if not 0 <= frame_off_threshold <= 1:
            raise ValueError("frame_off_threshold must be in [0, 1]")
        if release_debounce_frames < 1:
            raise ValueError("release_debounce_frames must be positive")
        if min_note_frames < 1:
            raise ValueError("min_note_frames must be positive")
        self.onset_decoder = OnsetVelocityNmsDecoder(
            num_keys, **onset_decoder_kwargs)
        self.frame_off_threshold = frame_off_threshold
    ...
    def forward(self, onset_probs, velmap, framemap, pthresh=None):
        if onset_probs.shape != framemap.shape:
            raise ValueError("Onset probabilities and frame map must match")
        if not bool(((framemap >= 0) & (framemap <= 1)).all()):
            raise ValueError("Frame map must contain probabilities in [0, 1]")
        onsets = self.onset_decoder(onset_probs, velmap, pthresh)
        columns = [
            "batch_idx", "key", "onset_idx", "offset_idx", "duration_frames",
            "prob", "vel", "frame_conf", "release_reason",
        ]
        if onsets.empty:
            return pd.DataFrame(columns=columns)

        width = framemap.shape[-1]
        rows = []
        grouped = onsets.sort_values(
            ["batch_idx", "key", "t_idx"]).groupby(["batch_idx", "key"])
        for (batch_idx, key), group in grouped:
            events = list(group.itertuples(index=False))
            curve = framemap[int(batch_idx), int(key)].detach().cpu()
            for event_idx, event in enumerate(events):
                onset_idx = int(event.t_idx)
                next_onset = (
                    int(events[event_idx + 1].t_idx)
                    if event_idx + 1 < len(events) else width
                )
                search_beg = min(
                    width, onset_idx + self.min_note_frames)
                search_end = next_onset
                reason = "reattack" if next_onset < width else "stream_end"
                if self.max_note_frames is not None:
                    cap = min(width, onset_idx + self.max_note_frames)
                    if cap < search_end:
                        search_end = cap
                        reason = "max_duration"
                ...
                below_count = 0
                for frame_idx in range(search_beg, search_end):
                    if float(curve[frame_idx]) < self.frame_off_threshold:
                        below_count += 1
                        if below_count >= self.release_debounce_frames:
                            offset_idx = (
                                frame_idx - self.release_debounce_frames + 1)
                            reason = "frame"
                            break
                    else:
                        below_count = 0
                ...
                offset_idx = max(
                    onset_idx + self.min_note_frames, offset_idx)
                active = curve[onset_idx:min(offset_idx, width)]
                frame_conf = (
                    float(active.mean()) if active.numel() else 0.0)
                rows.append({
