...
- Window sweep = `eval_offset.py` with `INFERENCE_CHUNK_SIZE` / `_OVERLAP` swept.

## Pod state at writing

Pod alive (lease ~until 2026-07-31). All eval JSONs on `/root/*.json`. Hung 0.25 s
sweep process killed. Compact results all pulled to local `results_eval/`.
Bootstrap 95% CIs already computed in the §1 summary (in-chat); per-file arrays are
in the JSONs if per-piece stats are needed later.
