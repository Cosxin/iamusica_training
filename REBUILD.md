...
- **Pedal head**: binary frame head caps at ~0.55-0.58 vs pedal GT (0.545 frozen /
  0.577 stem-unfrozen) — far below Mobile-AMT's 0.77. To match it you need a
  **regression offset head** (continuous sub-frame release), not the binary map.
  Not built.
- Codec (aptX/SBC) augmentation: onsets already codec-robust; codec hurts
  velocity/release, not onsets. See EVAL_FINDINGS §2/§6.
